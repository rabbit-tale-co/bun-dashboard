  ...
  <header class="relative w-full bg-background border-b border-backgroundSecondary shadow-lg">
    <section class="flex items-center justify-between max-w-screen-xl px-6 mx-auto">
      ...
      <ul class="flex items-center">
        <button @click="togglePluginsMenu" class="flex items-center max-lg:hidden">
          Plugins
          <OutlineChevronRight size="16" :class="{'ml-2 transition-transform duration-150': true, 'rotate-90': isPluginsMenuOpen, 'rotate-0': !isPluginsMenuOpen}" />
        </button>
        <button @click="toggleResourcesMenu" class="flex items-center max-lg:hidden ml-6">
          Resources
          <OutlineChevronRight size="16" :class="{'ml-2 transition-transform duration-150': true, 'rotate-90': isResourcesMenuOpen, 'rotate-0': !isResourcesMenuOpen}" />
        </button>
        <!-- Sub Navigation Menu for Plugins -->
        <section class="bg-background w-screen absolute left-0 top-[calc(100%+1px)] z-20 shadow-lg" v-if="isPluginsMenuOpen">
          <nav class="mx-auto max-w-screen-xl grid grid-cols-5">
            <section class="w-full h-full col-span-3 pt-8 pb-9 pr-8 grid grid-cols-2 gap-8">
              <div v-for="(item, i) in plugins" :key="i">
                <h4 v-if="item.category" class="text-foreground text-sm">{{ item.category }}</h4>
                  ...
                  <li v-for="(subitem, j) in item.subitems" :key="j">
                    <router-link v-if="!subitem.external" :to="subitem.path" class="block transition-colors text-sm text-foreground/40 hover:text-foreground/60">
                      <div class="flex gap-4">
                        <OutlineDiscord class="text-foreground mt-0.5"/>
                        <div class="flex flex-col flex-1">
                          <h4 class="text-foreground text-lg font-bold">{{ subitem.title }}</h4>
                          {{ subitem.description }}
                        </div>
                      </div>
                    </router-link>
        ...
        <!-- Sub Navigation Menu for Resources -->
        <section class="bg-background w-screen absolute left-0 top-[calc(100%+1px)] z-20 shadow-lg" v-if="isResourcesMenuOpen">
          <nav class="mx-auto max-w-screen-xl grid grid-cols-5">
            <section class="w-full h-full col-span-3 pt-8 pb-9 pr-8 grid grid-cols-2 gap-8">
        ...
        <Sheet>
          ...
          <SheetContent side="left" class="flex flex-col">
            <SheetHeader class="items-start">
              ...
              <Popover v-if="user">
                <PopoverTrigger class="h-10 flex items-center gap-2 ml-2">
                  <Avatar class="mr-2">
                    <AvatarImage :src="user.avatarUrl" :alt="user.username" />
                    <AvatarFallback>{{ avatarFallbackText }}</AvatarFallback>
                  </Avatar>
                  <span class="max-lg:hidden">{{ user.globalName }}</span>
                  <OutlineChevronRight size="16" class="rotate-90 transition-transform max-lg:hidden" />
                </PopoverTrigger>
